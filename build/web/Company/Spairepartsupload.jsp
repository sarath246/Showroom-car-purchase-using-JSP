...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Company\\SpaireParts\\";//path for storing image
   ...
   String str1="insert into tbl_sparepartsDetails(spareparts_name,sparepartstype_id,spareparts_cost,car_id,car_description,spareparts_image,company_id)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+fn+"','"+session.getAttribute("regid")+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Spareparts_details.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
