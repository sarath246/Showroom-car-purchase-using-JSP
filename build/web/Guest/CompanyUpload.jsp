<%-- 
    ...
    Created on : 7 Feb, 2020, 4:33:45 PM
    Author     : Sarath
--%>
...
if(isMultipart)
{
   ...
   Iterator itr=items_list.iterator();
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           if(field_name.equals("img_doctor"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Doctor\\DoctorImage\\";//path for storing image
            Random r=new Random();//to generate random number
             int r_num=r.nextInt(1111)+999;//values between this are generated
             
            fn="Doctor_"+r_num+ext;//generating random number for images so this name and format of imagewil be stored inside the server
   ...
   String str1="insert into tbl_doctor(doctor_name,doctor_gender,hospital_id,doctor_contact,doctor_email,qualification_id,doctor_username,doctor_password,sqty_id,doctor_answer,doctor_proof,doctor_photo)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+value[5]+"','"+value[6]+"','"+value[7]+"','"+value[9]+"','"+value[10]+"','"+proof+"','"+fn+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Login.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
