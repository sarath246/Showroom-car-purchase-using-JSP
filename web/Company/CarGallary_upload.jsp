...
if(isMultipart)
{
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            ...
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Company\\Car_gallary\\";//path for storing image
   ...
   String str1="insert into tbl_cargallary(car_id,car_caption,car_image)values('"+value[0]+"','"+value[1]+"','"+value[2]+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Car_gallary.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
