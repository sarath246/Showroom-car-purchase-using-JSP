<%-- 
    Document   : ShowroomCarsUpload
    Created on : 22 May, 2020, 7:22:23 PM
    Author     : Sarath
--%>
...
if(isMultipart)
{
   ...
   Iterator itr=items_list.iterator();
    ...
    while(itr.hasNext())//checking if there is any value in the position like rs.next()
    {
       ...
       else//when it is an image or multipart
       {
           ...
           if(field_name.equals("img_car"))//write the name of image here it is img_product
           {
           String ext=file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
            //setting path to store image
            File proj_path=new File(config.getServletContext().getRealPath("/"));
            String file_path=proj_path.getParentFile().getParentFile().getPath()+"\\Web\\Showrooms\\CarImages\\";//path for storing image
  ...
  String str1="insert into tbl_showroomcars(car_id,showroom_id,car_price,car_image)values('"+value[0]+"','"+session.getAttribute("regid")+"','"+value[1]+"','"+fn+"')";
    ...
    if(status==true)
    {
      %> 
       <script type="text/javascript">
            ...
            setTimeout(function(){window.location.href='Showroom_cars.jsp'},40);//40millisecend it go to next page
        </script>
     <%
    }
     }    
